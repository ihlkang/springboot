...
function showDiff(){
    ...
    $.each(oldJsonRule,function(){
        if(fieldDictJson[this.left]==null) {
            old += "条件["+this.alias + " " + this.operator + " " + this.right + "] 分[" + this.score + "]\n";
        }else{
            var crule=this;
            var dictstr=[];
            var rights;
            ...
            old += "条件["+this.alias + " " + this.operator + " " + dictstr+ "] 分[" + this.score + "]\n";
        }
    });
    var new_="";
    $.each(getDroolsJson(),function(){
        if(fieldDictJson[this.left]==null) {
            new_ += "条件["+this.alias + " " + this.operator + " " + this.right + "] 分[" + this.score + "]\n";
        }else{
            var crule=this;
            var dictstr=[];
            var rights;
            ...
            new_ += "条件["+this.alias + " " + this.operator + " " + dictstr+ "] 分[" + this.score + "]\n";
        }
    });
        ...
        $.ajax({
            ...
            success: function (response) {
                if (response.success) {
                    ...
                    $('.testResult').append("<tr><td colspan='2'><b>总分：" + resuleJson.score + "</b></td></tr>");
                } else {
                    $('.page-loading-overlay').addClass('loaded');
                    alert(response.data);
                }
