    ...
    jsMind.screenshot.prototype = {
        ...
        _draw_node:function(node){
            ...
            if(!!view_data.operation){
                this._draw_operation(view_data.operation);
            }
            if (!('background-image' in node.data)) {
                node.ready = true;
            }
        ...
        _draw_operation:function(expander){
            var ctx = this.canvas_ctx;
            ...
            var width = parseInt(style_width);
            var height = parseInt(style_height);
            var font = "normal normal 400 16px/18px Verdana,Arial,Helvetica,sans-serif";
            var bgcolor = css(ncs,'background-color');
            ctx.font="12px 微软雅黑";
            ctx.fillStyle = "#000";
            jcanvas.text_ellipsis(ctx, expander.innerHTML.replace("&lt;","<").replace("&gt;",">"), left, top, width, height);
            // ctx.font="30px 微软雅黑";
            // ctx.fillStyle="#fff";
            // ctx.fillText("Hello World",left,top);
        },
