...
(function($w){
    ...
    jsMind.draggable.prototype = {
        ...
        _magnet_shadow:function(node){
            if(!!node&&!node.node.topic.startsWith("通过")&&!node.node.topic.startsWith("转人工审核")){
        ...
        move_node:function(src_node,target_node,target_direct){
            ...
            if(!!target_node && !!src_node && !jsMind.node.inherited(src_node, target_node)){
                ...
                if(target_node.id!=src_node.parent.id){
                    //改变了父节点 重新设置表达式
                    this.jm.view.clean_nodes_operation(src_node,target_node);
                }
