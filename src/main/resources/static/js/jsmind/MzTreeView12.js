...
MzTreeView.prototype.nodeToHTML = function(node, AtEnd)
{
    ...
    var id   = node.id;
    var HCN  = node.hasChild||this.getAttribute(source, "isFolder")=="true", isRoot = node.parentId=="0";
    ...
    if(!isRoot)
    {
        ...
        {
            node.iconExpand  = HCN&&node.hasChild ? AtEnd ? "PM2" : "PM1" : AtEnd ? "L2" : "L1";
    ...
    HTML += "<A "+
        ...
        "onclick=\"return "+ this.name +".nodeClick('"+ id +"')\">"+ " &nbsp;" + HTMCHK + node.T +
        "</A>"+ HTMTD +"</NOBR></DIV>";
    if(isRoot && node.T=="") HTML = "";

    HTML = "\r\n<SPAN class='field' id='"+ this.name +"_tree_"+ id +"'>"+ HTML ;
