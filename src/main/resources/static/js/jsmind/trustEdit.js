...
function add(row) {
    if ($.inArray(row.left, array) == -1) {
        ...
        //添加属性框div内标题栏
        $(propDiv).append("<table width='100%'><tr><td width='90%'>" + row.alias + "</td>" +
...
function showDiff() {
    ...
    $.each(oldJsonRule, function () {
        if (fieldDictJson[this.left] == null) {
            old += "条件[" + this.alias + " " + this.operator + " " + this.right + "] 额度[" + this.score + "]\n";
        } else {
            var crule = this;
            var dictstr = [];
            var rights;
            ...
            $.each(rights, function () {
                ...
                $.each(dicts, function () {
                    if (this.value == cright) {
                        if (fieldDictJson[crule.left] == 19) {
            ...
            old += "条件[" + this.alias + " " + this.operator + " " + dictstr + "] 额度[" + this.score + "]\n";
        }
    });
    var new_ = "";
    $.each(getDroolsJson(), function () {
        if (fieldDictJson[this.left] == null) {
            new_ += "条件[" + this.alias + " " + this.operator + " " + this.right + "] 额度[" + this.score + "]\n";
        } else {
            var crule = this;
            var dictstr = [];
            var rights;
            ...
            new_ += "条件[" + this.alias + " " + this.operator + " " + dictstr + "] 额度[" + this.score + "]\n";
        }
    });
...
function doSave() {
    ...
    if (confirm("保存前请先使用测试是否正确，是否已测试？")) {
        ...
        var param = {
            ...
            approveEffectTime: $('#approveEffectTime').val()
        };
        $.ajax({
            type: "POST",
            url: basePath + "web/blackfish/drools/doScoreEdit",
...
function checkDroolsJson() {
    ...
    $('#rules').find(".subrow").each(function () {
        ...
        if (this.id == "jzed_subrow") {
            if ($(this).find('.editInput')[0].value == "" || isNaN($(this).find('.editInput')[0].value)) {
                alert('基准额度请输入数值');
                ck = false;
                return;
            }
...
function test() {
    if (checkTest() && checkDroolsJson()) {
        ...
        $.ajax({
            type: "POST",
            url: basePath + "web/blackfish/drools/doScoreEditTest",
            ...
            success: function (response) {
                if (response.success) {
                    ...
                    $('.testResult').append("<tr><td colspan='2'><b>总额度：" + resuleJson.score + "</b></td></tr>");
                } else {
                    $('.page-loading-overlay').addClass('loaded');
                    alert(response.data);
                }
    ...
    $('#rules').find(".subrow").each(function () {
        if (this.id == "jzed_subrow") {
            jsonarray.push({
                ...
                "operator": "!=",
                "right": "-1",
                "score": $(this).find('.editInput')[0].value
            });
        } else {
            if ($($(this).find('.editInput')[1]).val() instanceof Array) {
                jsonarray.push({
    ...
    if (isTest) {
        ...
        propRowDivOlDivMultiple.multiselect({
            ...
            onDropdownShow: function (event) {
                ...
                if (fieldDictJson[$(cuttentMu).attr("prop")] == 19) {
                    includeSelectAllOption_ = true;
                    enableFiltering_ = true;
...
function showTest() {
    if (checkDroolsJson()) {
        jQuery('#modal-test').modal('show', {backdrop: 'static'});
    }
}
    ...
    $.each(city, function () {
        ...
        if (this.label.indexOf(" ") != -1) {
            subgroup.children.push({
                "label": this.label.substring(this.label.indexOf(" "), this.label.length),
                "title": this.title,
                "value": this.value
            });
        } else {
