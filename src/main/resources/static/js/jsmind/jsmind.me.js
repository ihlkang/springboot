...
(function($w){
    ...
    // an noop function define 创建日志对象 防止浏览器不兼容
    var _noop = function(){};
    var logger = (typeof console === 'undefined')?{
        log:_noop, debug:_noop, error:_noop, warn:_noop, info:_noop
    }:console;

    // check global variables 防止重复加载 例如js引用两次
    if(typeof $w[__name__] != 'undefined'){
        logger.log(__name__+' has been already exist.');
        return;
    }

    // shortcut of methods in dom 定义一些常用的方法缩写，类似jquery 不依赖jquery
    var $d = $w.document;
    ...
    //防止浏览器不兼容startsWith方法
    if(typeof String.prototype.startsWith != 'function'){String.prototype.startsWith=function(p){return this.slice(0,p.length)===p;};}

    var DEFAULT_OPTIONS = {
        container : '',   // id of the container    画图容器div的id
        editable : false, // you can change it in your options  是否可编辑
        theme : null,          //样式
        mode :'full',     // full 父节点 or side 根节点
        support_html : true,  //内容是否支持html格式
    ...
    var jm = function(options){
        ...
        this.options = opts;//一些样式参数
        this.inited = false;//防止重复加载
        this.mind = null;//标签集合
        this.event_handles = [];//快捷键
        this.init();
    };
    ...
    jm.format = {
        ...
        node_array:{
            example:{
                ...
                "format":"node_array",
                "data":[
                    {"id":"root","topic":"开始","left":"droolsstart_m", "isroot":true}
                ]
            },
    ...
    jm.prototype={
        init : function(){
            ...
            this.data = new jm.data_provider(this);//用来加载、解析 数据 生成this.mind
            this.layout = new jm.layout_provider(this, opts_layout);//布局标签
            this.view = new jm.view_provider(this, opts_view);//根据数据生成标签
            this.shortcut = new jm.shortcut_provider(this, opts.shortcut);//快捷键
        ...
        scale:function(x,y){
            this.view.canvas_ctx.scale(x,y);
        },
        ...
        add_move_node:function(parent_node, nodeid, topic, data,beforeid,direction){
            if(this.get_editable()){
                ...
                if(!!node){
                    ...
                    var node = this.mind.move_node(nodeid,beforeid,parent_node.id,direction);
                    this.view.update_node(node);
                    this.layout.layout();
                    this.view.show(false);
                    this.view.clean_nodes_operation(node,parent_node);
        ...
        copy_node:function(parent,node,isShow){
            if(this.get_editable()){
                var uuid=jm.util.uuid.newid();
                node.id=uuid
                var newNode=this.mind.add_node(parent, uuid, node.topic, node.data);
                this.view.add_node(newNode);
                if(node.children!=null&&node.children.length>0){
                    for(var i=0;i<node.children.length;i++){
                        this.copy_node(uuid,node.children[i],false);
                    }
                }
                if(!!isShow){
                    this.layout.layout();
                    this.view.show(false);
                    this.view.reset_node_custom_style(newNode);
                    this.expand_node(parent);
                    this.select_node(newNode);
                    this.invoke_event_handle(jm.event_type.edit,{evt:'add_node',data:[parent.id,newNode.id,newNode.topic,newNode.data],node:newNode.id});
                }
            }else{
                logger.error('fail, this mind map is not editable');
                return null;
...
// ============= data provider =============================================
    ...
    jm.view_provider.prototype={
        ...
        get_binded_nodeid:function(element){
            ...
            if(tagName == 'jmnode' || tagName == 'jmexpander' || tagName == 'jmoperation'){
        ...
        is_operation:function(element){
            return (element.tagName.toLowerCase() == 'jmoperation');
        },
        ...
        expand_size:function(){
            ...
            var min_width = min_size.w + this.opts.hmargin*2+150;
            var min_height = min_size.h + this.opts.vmargin*2+50;
            ...
            if(this.size.w != client_w){
                $w.setTimeout(function(){
                    try{
                    $('.jsmind-inner')[0].scrollTo($('.jsmind-inner')[0].scrollWidth,$('.jsmind-inner')[0].scrollTop);
                    }catch(e){}
                },0);
            }
        ...
        reset_nodes_operation:function(node){
            if(!node.isroot&&!node.parent.isroot) {
                var view_data = node._data.view;
                if (node.data.operation == "" || node.data.operation == null) {
                    view_data.operation.operation = "[{\"left\":\"" + node.parent.data.left + "\"}]";
                    $t(view_data.operation, "-");
                } else {
                    view_data.operation.operation = node.data.operation;
                    var eo=eval("("+node.data.operation+")");
                    if(eo.length==1){
                        setLineRight(view_data.operation,eo[0].left,eo[0].operation,eo[0].right);
                        // $t(view_data.operation, e[0].operation+" "+ e[0].right);
                    }
                    if(eo.length==2){
                        setLineRight2(view_data.operation,eo[0].left,eo[0].operation,eo[0].right,eo[1].operation,eo[1].right);
                    }
                }
            }
        },
        clean_nodes_operation:function(node,tagNode){
            var view_data = node._data.view;
            node.data.operation ="[{\"left\":\"" + tagNode.data.left + "\"}]";
            $t(view_data.operation, "-");
        },
        init_nodes:function(){
            var nodes = this.jm.mind.nodes;
        ...
        create_node_element:function(node,parent_node){
            ...
            if(node.isroot){
                ...
                if(node.topic.startsWith("通过")||node.topic.startsWith("转人工审核")){
                    d.className = 'pass';
                }else if(node.topic=='预警'){
                    d.className = 'alarm';
                }else if(node.topic=='冻结'){
                    d.className = 'freeze';
                }
		 if (node.data.left.startsWith("custom-")){
                    d.className = 'black';
                }
                //伸缩控件
                ...
                //表达式
                var d_o = $c('jmoperation');
                $t(d_o,'-');
                d_o.setAttribute('nodeid',node.id);
                d_o.style.visibility = 'hidden';
                d_o.style.zIndex = 2;
                parent_node.appendChild(d_o);
                view_data.operation = d_o;
                d_o.node=node;
            }
            if (!!node.topic) {
            ...
            //设置表达式
            this.reset_nodes_operation(node)
        ...
        remove_node:function(node){
            ...
            if(node._data.view){
                ...
                var operation = node._data.view.operation;
                this.e_nodes.removeChild(element);
                this.e_nodes.removeChild(expander);
                this.e_nodes.removeChild(operation);
                node._data.view.element = null;
                node._data.view.expander = null;
                node._data.view.operation = null;
            }
        },
        ...
        show_nodes:function(){
            ...
            for(var nodeid in nodes){
                ...
                if(!this.layout.is_visible(node)){
                    ...
                    view_data.operation.style.display = 'none';
                    continue;
                }
                ...
                if(!node.isroot&&!node.parent.isroot){
                    ...
                    // }else{
                    //     view_data.operation.operation=node.data.operation;
                    //     $t(view_data.operation,view_data.operation.operation);
                    // }
                    p_expander= this.layout.get_expander_point(node);
                    view_data.operation.style.left = (_offset.x + p_expander.x-node._data.view.width-110) + 'px';
                    view_data.operation.style.top = (_offset.y + p_expander.y-10) + 'px';
                    view_data.operation.style.display = '';
                    view_data.operation.style.visibility = 'visible';
                }else{
                    if(!!view_data.operation){
                        view_data.operation.style.display = 'none';
                    }
                }
            }
        ...
        draw_line:function(pin,pout,offset,canvas_ctx){
            var canvas =  $c('canvas');
            this.e_nodes.appendChild(canvas);
            $(canvas).addClass("jmcanvas");
            canvas.style.left=(offset.x+pin.x-1)+"px";
            if(pout.y<0&&pin.y<0){
                canvas.style.top=(offset.y+(pout.y<pin.y?pout.y:pin.y)-1)+"px";
            }else if(pout.y<0){
                canvas.style.top=(offset.y+pout.y-1)+"px";
            }else if(pin.y<0){
                canvas.style.top=(offset.y+pin.y-1)+"px";
            }else{
                if(pout.y>pin.y){
                    canvas.style.top=(offset.y+pin.y-1)+"px";
                }else{
            ...
            canvas.width=Math.abs(pin.x-pout.x)+1;
            canvas.height=Math.abs(pin.y-pout.y)+1;
            ...
            jm.util.canvas.bezierto(ctx,x,y,x2,y2);
        },
    };
    ...
    jm.shortcut_provider.prototype = {
        ...
        handle_delnode:function(_jm,e){
            ...
            var selected_node = _jm.get_selected_node();
                ...
                if(!!selected_node && !selected_node.isroot){
                    if(confirm("确认删除【"+selected_node.topic+"】？")){
